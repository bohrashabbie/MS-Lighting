...
export default async function HomePage() {
  ...
  // pick a representative image for a category — hi-res local render first,
  // then its own catalogue image, else the first product's
  const catImg = (c: ProductCategory, i: number) => {
    const r = RENDERS.find((r) => r.categorySlug === c.slug);
    return r?.src || imageUrl(c.image_url) || imageUrl(details[i]?.products[0]?.image_url) || null;
  };

  // Dedicated hero art (not tied to a catalogue model_code) — a reliable hero, never the soft PDF scan.
  const heroImg = "/renders/hero-spotlight.png";
      ...
      <section className="hero">
        <div className="inner">
          ...
          <div className="stage reveal">
            ...
            {heroImg ? (
              <Image src={heroImg} alt="MS Lighting fixture" width={1562} height={1800} priority quality={92} sizes="(max-width:920px) 90vw, 46vw" />
            ) : (
